...
private val tools=listOf(
    Tool("proxy","⇄","代理中心","多协议全局 VPN、扫码导入、账号认证与远程 DNS"),
...
@Composable fun ToolPage(activity:MainActivity,tool:Tool,back:()->Unit){
    ...
    Column(Modifier.fillMaxSize().background(bg).padding(20.dp)){
        ...
        when(tool.id){
            "proxy"->{
                OutlinedTextField(input,{input=it},Modifier.fillMaxWidth(),label={Text("代理节点／分享链接")},minLines=4);Spacer(Modifier.height(12.dp))
                Button(onClick={output=runCatching{val n=SingBoxConfig.parse(input);"节点有效\n协议：${n.protocol.uppercase()}\n服务器：${n.host}:${n.port}"}.getOrElse{"格式无效：${it.message}"}},Modifier.fillMaxWidth()){Text("检查节点")}
                Button(onClick={runCatching{SingBoxConfig.parse(input)}.onSuccess{prefs.edit().putString(AuroraVpnService.KEY_NODE,input.trim()).apply();val permission=VpnService.prepare(activity);if(permission==null)startVpn()else vpnPermission.launch(permission)}.onFailure{output="无法连接：${it.message}"}},Modifier.fillMaxWidth()){Text("连接 · 接管全局网络")}
                ...
                Text("真实支持 SOCKS5、Shadowsocks、VMess、VLESS、Trojan；包含 TLS/Reality、WebSocket、gRPC、HTTP/2 常用传输参数。",color=muted)
            }
