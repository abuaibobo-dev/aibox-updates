...
@Composable fun Page(a:MainActivity,t:Tool,back:()->Unit){var input by remember{mutableStateOf("")};var output by remember{mutableStateOf("")};val picker=rememberLauncherForActivityResult(ActivityResultContracts.GetContent()){uri:Uri?->if(uri!=null){val image=InputImage.fromFilePath(a,uri);if(t.id=="ocr")TextRecognition.getClient(ChineseTextRecognizerOptions.Builder().build()).process(image).addOnSuccessListener{output=it.text.ifBlank{"未识别到文字"}}.addOnFailureListener{output="识别失败：${it.message}"}else BarcodeScanning.getClient().process(image).addOnSuccessListener{val value=it.firstOrNull()?.rawValue.orEmpty();if(t.id=="proxy"&&value.isNotBlank())input=value;output=value.ifBlank{"未识别到二维码"}}.addOnFailureListener{output="识别失败：${it.message}"}}};Column(Modifier.fillMaxSize().background(bg).padding(20.dp)){TextButton(onClick=back){Text("← 返回")};Text("${t.icon}  ${t.title}",style=MaterialTheme.typography.headlineMedium,color=Color.White);Text(t.desc,color=muted);Spacer(Modifier.height(24.dp));when(t.id){"proxy"->{OutlinedTextField(input,{input=it},Modifier.fillMaxWidth(),label={Text("节点／分享链接")},minLines=4);Spacer(Modifier.height(12.dp));Button(onClick={output=runCatching{val u=URI(input.trim());if(u.scheme !in listOf("http","https","socks5","vmess","vless","trojan","ss"))error("不支持的协议");"节点格式有效：${u.scheme}\n服务器：${u.host?:"加密分享内容"}"}.getOrElse{"格式无效：${it.message}"}},Modifier.fillMaxWidth()){Text("检查节点")};OutlinedButton(onClick={picker.launch("image/*")},Modifier.fillMaxWidth()){Text("扫描二维码导入")};OutlinedButton(onClick={a.getSharedPreferences("proxy",0).edit().putString("last",input).apply();output="节点已保存在本机"},Modifier.fillMaxWidth()){Text("保存节点")};OutlinedButton(onClick={input=a.getSharedPreferences("proxy",0).getString("last","").orEmpty()},Modifier.fillMaxWidth()){Text("载入已保存节点")};OutlinedButton(onClick={a.startActivity(Intent(Settings.ACTION_VPN_SETTINGS))},Modifier.fillMaxWidth()){Text("打开 Android VPN 设置")};Text("支持 HTTP(S)、SOCKS5、VMess、VLESS、Trojan、Shadowsocks；下一阶段接入 GPLv3 libbox 后启用全局 TUN、分应用代理、路由和 DNS。",color=muted)};"system"->{val rt=Runtime.getRuntime();output="设备：${android.os.Build.MANUFACTURER} ${android.os.Build.MODEL}\nAndroid：${android.os.Build.VERSION.RELEASE} (API ${android.os.Build.VERSION.SDK_INT})\nCPU 架构：${android.os.Build.SUPPORTED_ABIS.joinToString()}\n应用可用内存：${(rt.maxMemory()-rt.totalMemory()+rt.freeMemory())/1048576} MB";Button(onClick={output=output},Modifier.fillMaxWidth()){Text("刷新检查")}};else->Button(onClick={picker.launch("image/*")},Modifier.fillMaxWidth()){Text(if(t.id=="ocr")"选择图片提取文字" else "选择二维码图片")}};if(output.isNotBlank()){Spacer(Modifier.height(20.dp));Card(colors=CardDefaults.cardColors(containerColor=panel)){Text(output,Modifier.fillMaxWidth().padding(16.dp),color=Color.White)}}}}
